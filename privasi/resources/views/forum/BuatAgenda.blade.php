...
<div class="container">
	<form action="{{ url('/simpanagenda') }}" method="POST">
		{{ csrf_field() }}
	    ...
	    <input type="hidden" name="postid" value="{{ $thread->post_id }}">	
	    <div class="form-group @if($errors->has('komunitas')) has-error has-feedback @endif">
			<label for="komunitas">Untuk Komunitas</label>
			<input type="text" class="form-control" name="komunitas" placeholder="Nama Komunitas" value="{{ $thread->title }}">
